/*
  PROJECT NAME: ESP32 Fingerprint Doorlock With Database Interface
  AUTHOR: Donsky
...
// GPIO Pins for rows
byte pin_rows[ROW_NUM] = {19, 18, 5, 32};
// GPIO Pins for columns
byte pin_column[COLUMN_NUM] = {13, 14, 15, 33};
...
// interval before the fingerprint sensor is disabled
unsigned long prevAuthenticatedMillis = 0;
const long SCAN_INTERVAL_TIME = 10000;
...
bool parseJson(String jsonDoc)
{
  ...
  {
    prevAuthenticatedMillis = millis();
    return true;
  }
}
...
void loop()
{
  ...
  // Serial.println(key);
  if (is_authenticated && (currentMillis - prevAuthenticatedMillis) < SCAN_INTERVAL_TIME)
  {
